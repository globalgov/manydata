...
#' @param repo the github repository to track, e.g. "jhollway/migraph"
#' @importFrom httr GET
...
plot_releases <- function(repo){
  ...
  if (!is.data.frame(repo)){
    ...
    get_releases <- function(repo){
      ...
      code_milestone <- function(tag_name){
        ...
        test <- lapply(stringr::str_split(stringr::str_remove(tags, "v"),"\\."), function(x) as.numeric(x))
      ...
      df$date <- lubridate::ymd(stringr::str_replace(df$date, "-[:digit:]*$", "-01"))
  ...
  if(length(year_date_range)==1) year_date_range <- c(min(df$date) - months(month_buffer), max(df$date) + months(month_buffer))
