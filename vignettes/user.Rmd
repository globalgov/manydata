...
Let us say that we wish to download the `{qStates}` package,
...
specifying which package we want to 'get', 'get_packages("globalgov/qStates")'.

For now, let's work with the Roman Emperors database included in qData. 
...
However, if we want a more detailed summary of the various levels of data 
...
If we were specifically interested in the one dataset of the
database, we can further specify the arguments. 
...
and tell it to operate on just the "Beg" variable, wich represents 
...
Now that we have visualized the data and looked at some of the different inferences
drawn using different datasets, let us examine how to select and collapse databases.
`{qData}` contains the `consolidate()` which facilitates collapsing multiple
datasets into a dataframe for analysis.

`consolidate()` collapses a set of (q)datasets, or a database,
