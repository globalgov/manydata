...
test_that("standardise_dates() treats reverse ordered dates correctly", {
  expect_equal(as.character(standardise_dates(c("30.1.1874", "31.12.1879", "6/10/2012", "9.4.1939", "12/5/1993", "10.30.93"))), 
               c("1874-01-30", "1879-12-31", "2012-10-06", "1939-04-09", "1993-05-12", "1993-10-30"))
...
#   expect_match(as.character(min(standardise_dates(c("2010", "2010-01")))), c("2010-01-01", "2010-01-01"))
#   expect_match(as.character(max(standardise_dates(c("2010", "2010-01")))), c("2010-12-31", "2010-01-31"))
# })
...
test_that("standardise_dates() treats multiple inconsistent dates correctly",{
  ...
  dat <- data.frame(date = as.character(c("12.10.2010", " ", "20/10/1993", "2010.10.12","2010/03/30", "10/12/1993", "NA", "93-10-20")))
  expect_equal(as.character(standardise_dates(dat$date)), c("2010-10-12", NA, "1993-10-20","2010-10-12", "2010-03-30", "1993-12-10", NA, "1993-10-20"))
  dat1 <- data.frame(date = as.Date(c("1351-08-01", "1353-10-20", "1403-06-27", "1407-03-10",
                                        "1656-07-17", "NA", "1867-04-29"))) # Example of errors from datasets in qEnviron
  expect_equal(as.character(standardise_dates(dat1$date)), c("1351-08-01", "1353-10-20", "1403-06-27", "1407-03-10",
                                                               "1656-07-17", NA, "1867-04-29"))
  dat2 <- data.frame(date = c("Sep 12, 2009", "Oct 1, 2019", "Nov 13, 1998", "NA", "May 13, 2003"))
  expect_equal(as.character(standardise_dates(dat2$date)), c("2009-09-12", "2019-10-01", "1998-11-13", NA, "2003-05-13"))
})
