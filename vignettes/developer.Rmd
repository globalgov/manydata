...
two-dimensional datasets into three-dimensional databases that resembles a data cube (a list object where entries are dataframes).
...
Corrected datasets can be connected to form a three dimensional database that resembles a data cube. The function creates a database in a data folder to store corrected datasets,while providing a citation template and a test template. The citation template helps cite the source of the raw data and and describe the variables in the corrected dataset. The test template makes sure the corrected dataset is consistent with {qData} guidelines.

```{r eval=FALSE}
qData::export_data(COW, "states")
```
