...
many packages universe, we have developed the `call_packages()` function.
...
As well, the `call_sources()` function displays sources, URLs, and variable changes for
the datasets within a database within a many package.
...
and sources, we can use `compare_` family of functions.
...
The `compare_overlap()` function returns a tibble with dataset overlaps within the database.
...
The `compare_missing()` function returns a tibble with missing observations in datasets within database.
...
Finally, both the `compare_categories()` and the `plot_categories()` functions help researchers identify how datasets in database relate to one another.
