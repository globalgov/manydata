...
two-dimensional datasets into three-dimensional databases that resembles a data cube.
...
These include, for example, the `transmutate()` function which is between between dplyr's 
...
package using the `export_data()` function. The function creates a data folder to store 
corrected datasets, while providing a citation template and a test template. The citation 
template helps cite the source of the raw data and and describe the variables in the corrected 
dataset. The test template makes sure the corrected dataset is consistent with {qData} guidelines.
