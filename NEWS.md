...
* Closed #78 by creating a new function `report_data()` to create metadata dataframes for each level of data (Package, Database, and Dataset)
...
* Fixed #95 by simplifying `standardise_dates()` to make sure it works with all types of date variables within different datasets
* closed #93 by setting up internal helper functions for `standardise_dates()` 
* closed #92 by tweaking `export_data()` to document datasets at the database level
  * The documentation generated by export_data() references the newly created report_data() function to generate metadata dataframes at the desired level (Package, Database, Dataset)
