test_that("resolve works", {
  test <- dplyr::tibble(var = c("1995-10-30", "1816-01-01", NA)) 
  expect_equal(resolve_min(test$var), c("1995-10-30", "1816-01-01", NA))
  expect_equal(resolve_max(test$var), c("1995-10-30", "1816-01-01", NA))
  expect_equal(resolve_mean(test$var),c("1995-10-30", "1816-01-01", NA))
  expect_equal(resolve_median(test$var), c("1995-10-30", "1816-01-01", NA))
  expect_equal(resolve_mode(test$var), c("1995-10-30", "1816-01-01", NA))
})
# Passes now that I changed the tibble that it is tested with to fit reality
...
test_that("resolve treats date ranges correctly", {
  test <-  dplyr::tibble(var = c("2010-01-01:2010-12-31", "1995-10-30", "1816-01-01", "1816-01-01:1916-01-01", NA)) 
