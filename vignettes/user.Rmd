...
Now that we have visualized the data and looked at some of the different inferences drawn 
using different datasets `{qStates}`, let us examine how to select and collapse it. 
`{qData}` contains the `consolidate()` which facilitates collapsing 
multiple datsets in a dataframe into one for analysis.

`consolidate()` collapses a set or database of (q)datasets into a single dataset 
with some combination of the rows, columns, and observations of the parent datasets.
The function includes separate arguments for the rows and columns, as well as 
for how to resolve conflicts in observations across datasets.
...
```{r}
consolidate(qStates::states, rows = "any", cols = "any", resolve = "coalesce", key = "ID")
consolidate(qStates::states, rows = "every", cols = "every", resolve = "coalesce", key = "ID")
```
