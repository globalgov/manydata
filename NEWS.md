...
* Updated and fixed bugs with `standardise_dates()` function and tests
  * Closed #73 `standardise_dates()` function by making it works with `lubridate::as_date`, AD and historical dates
...
* Updated `export_data()` to reflect new database and dataset structure 
  ...
  * The documentation generated by export_data() references the newly created `report_data()` function to 
