...
If we use the hand coded duplicates to identify all duplicates in the dataset we find rates of true positives and false positives in a receiver operating characteristics (ROC) graph we find that it matches perfectly as the area under curve (AUC) is maximized (equals 1). That is, we identify all true positives and no false positives.   
...
In reality, we often do not get a perfect score as we do not know all the true duplicates ...
...
If we randomly cretae a variable with 200 values, we can also use to find duplicates.
The random scoares improve a bit the AUC scores by matching some true positives, but they also generate some false positives.
