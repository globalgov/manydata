...
If we use the hand coded duplicates to identify all duplicates in the dataset, the rates of true positives and false positives are represented the receiver operating characteristics (ROC) graph below. The area under the curve (AUC score) represented in dark gray in the graph is maximized (equals 1). That is, we identify all true positives and no false positives.   

```{r unique, warning=FALSE, message=FALSE}
agreements$unique <- 0
library(WVPlots)
WVPlots::ROCPlot(agreements, "duplicate", "duplicate", 1, title = "Unique")
```

In reality, we often do not get a perfect score as we do not know all the true duplicates. The next sections illustrate how the AUC score varies depending on the method used to deal with duplicates.
...
That is, the graph illustrate that we find zero false positives as well as zero true positives.
...
The random scores improve a bit the AUC scores by matching some true positives, but they also generate a few false positives where the blue curve goes under the black curve.
...
Strict match generates no false positives but it only captures a few of the true positives.
...
2. errors on the side of false positives depending on max. distance specification (when max. distance increases, chances of generating false positives increases as well)
