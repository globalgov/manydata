...
Packages in the many universe have the advantage to facilitate comparison 
...
specifying which package we want to 'get', 'get_packages("globalgov/manystates")'.
...
However, the real value of manydata packages is that multiple datasets
relating to the same phenomenon are presented together.

## Comparing data

First of all, we want to understand what the differences are between datasets in a database.
...
By default, `consolidate()` resolves conflicts between observations with the 'coalesce' method that takes the first non NA value available.
Users can, however, choose how they want to resolve conflicts between observations with several other methods:
...
Users can also specify how conflicting observations in different variables will be resolved differently.
